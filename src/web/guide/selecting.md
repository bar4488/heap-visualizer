...
Shift-drag on either strip, or drag vertically in the Events panel. The
selection is transient; Escape clears it.

- It is made in one domain and mirrored into the other, drawn on both strips and
  in the Events gutter. The same 200 events are a sliver in time and a wide band
  in events.
...
- **Tag allocs** takes the allocations created in the range. **Tag freed** takes
  the creators of the `F` and `R` events in it — "what died during this dip?"
- **Crop** restricts attention to allocations created in the seq window. It
  ...
  empty the map. It persists until you clear it from the toolbar ✂ pill.
...
- id, address range, requested size, `usable` if the producer sent it, site,
  thread, birth `seq`/`t`, death `seq`/`t` or `never (leak?)`, stack.
- Any field the producer added that this format does not define, verbatim. The
  two `pool` records at the end of
